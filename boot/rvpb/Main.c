...
static void Memory_write_stress(void);
...
// Randomly write 16-byte chunks within absolute 11KB~12KB region while the OS runs.
#define STRESS_REGION_BASE   (11 * 1024)  // 0x2C00
#define STRESS_REGION_SIZE   (1 * 1024)   // 1KB window
static volatile uint8_t mem_stress_buffer[STRESS_REGION_SIZE] __attribute__((aligned(16), section(".stress")));
...
static void Memory_write_stress(void)
{
    ...
    static uint32_t stress_rng = 0x12345678;

    // LCG for pseudo-random offset selection
    stress_rng = stress_rng * 1664525u + 1013904223u;
    uint32_t offset = stress_rng % (STRESS_REGION_SIZE - 16); // keep 16B window in range

    for (uint32_t i = 0; i < 16; i++)
    {
        mem_stress_buffer[offset + i] = pattern + (uint8_t)i;
    }

    pattern += 0x1F; // vary pattern so writes are not optimized away
}
