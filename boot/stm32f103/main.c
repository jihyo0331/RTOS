...
static void Kernel_Init(void) {
	/*
		...
		if(NOT_ENOUGH_TASK_NUM == taskId) {
			debug_printf("Polling_task cretion fail\n");
		}

		taskId = Kernel_task_create(Host_comm_task);
		if(NOT_ENOUGH_TASK_NUM == taskId) {
			debug_printf("Host_comm_task cretion fail\n");
		}
...
static void SystemClock_Config(void) {
	...
	RCC_OscInitStruct.PLL.PLL.PLLMUL = RCC_PLL_MUL6;
	...
	RCC_ClkInitStruct.ClockType = RCC_CLOCKTYPE_HCLK|RCC_CLOCKTYPE_SYSCLK
		|RCC_CLOCKTYPE_PCLK1|RCC_CLOCKTYPE_RCLK2;
